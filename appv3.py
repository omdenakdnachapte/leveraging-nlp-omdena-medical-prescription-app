...
# and FAISS similarity search. Designed for interactive use with Streamlit and Google Drive integration.
...
def load_model_and_artifacts(num_classes):
    # Files are now assumed to be in the root directory
    with open("tfidf_vectorizer.pkl", "rb") as f:
        tfidf = pickle.load(f)
    st.write("TF-IDF Fitted:", hasattr(tfidf, 'idf_'))  # Check if TF-IDF is fitted
    input_dim = len(tfidf.vocabulary_)
    st.write("TF-IDF input dimension:", input_dim)  # Verify input dimension
...
def predict(symptoms, model, tfidf, label_encoder, faiss_index, df, confidence_threshold=0.6):
    ...
    symptoms_vec = symptoms_tfidf / (np.linalg.norm(symptoms_tfidf, keepdims=True) + 1e-6)  # Avoid division by zero
...
# Main app
def main():
    # Define paths (files are now in the root directory)
    data_path = "processed_diseases-priority.csv"

    # Load data and model
    try:
        num_classes = len(pd.read_csv(data_path)["Disease"].unique())
        model, tfidf, label_encoder, faiss_index = load_model_and_artifacts(num_classes)
            ...
            with st.spinner("Analyzing symptoms..."):
                ...
                try:
                    ...
                    with col2:
                        ...
                        st.warning(f"**Similar Disease:** {similar_disease} (Similarity: {similarity:.4f})")
    ...
    # Display files in model directory
    st.write("Files in model directory:", os.listdir("omdenakdnachapte/leveraging-nlp-omdena-medical-prescription-app"))
