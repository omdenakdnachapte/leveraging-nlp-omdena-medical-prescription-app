...
def load_model_and_artifacts(num_classes):
    try:
        ...
        try:
            ...
            st.error(f"Failed to load FAISS index: {faiss_error}. The index file may be corrupted or incompatible with the current FAISS version.")
            raise  # Re-raise to halt execution and notify the user
